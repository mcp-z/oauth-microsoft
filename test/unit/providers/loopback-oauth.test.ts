 ...
 * server-managed token storage and loopback redirect handling (RFC 8252), and for
 * authMiddleware() behavior across different account scenarios.
 *
...
before(async () => {
  ...
  const tempTokenStorePath = path.resolve('.tmp', `oauth-microsoft-loopback-${Date.now()}.json`);
